...
export async function checkTerraformVersion(resolvedVersion: string, terraformWorkingDir: string, miseVersion?: string | null): Promise<void> {
    ...
    console.log(pc.green(`✓ Terraform version matches (${localVersion})`));
}
...
export async function handleAction(step: Step, isDryRun: boolean, workflow: Workflow, workingDir: string, stepId: string, workflowFile: string, jobName: string, context: WorkflowContext): Promise<'success' | 'skipped' | 'failed'> {
    ...
    if (!fs.existsSync(mocksDir)) {
        console.log(`   No mock directory found for ${actionNameWithoutVersion} at ${mockFilePath}- skipping action`);
        return 'skipped';
    }
    ...
    try {
        ...
        console.log(pc.green(`✓ Action ${actionNameWithoutVersion} completed successfully with mock`));
        return 'success';
